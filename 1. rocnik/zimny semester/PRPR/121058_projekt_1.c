    ...
    if (*f == NULL) {                                   /* ak sa nepodarilo otvorit, vypise print*/
        ...
        if (*n_was_open == 1) {                         /*ak uz bola zavolana funkcia n*/
            if (*z_was_open == 1) {                     /*ak uz bola zavolana funkcia z*/
                ...
                for (int n = 0; n < *total; n++) {                      /*v range total (pocet "stróf databazy = 57)*/ /*printujeme nacitane udaje z alokovanych poli*/
                    printf("ID Cislo mer. osoby %s", (*rc)[n]);
                    printf("Mer. modul: %s", (*modul)[n]);
                    printf("Typ mer. veliciny: %s", (*velicina)[n]);
                    printf("Hodnota: %s", (*hodnota)[n]);
...
void n_funkcia(FILE **f, char ***rc, char ***modul, char ***velicina, char ***hodnota, char ***cas, char ***datum,
    ...
    if (*v_was_open == 0) {                             /*ak V fukncia nebola zavolana*/
        ...
        *total = (*total / 7)+1;                        /*celkovy pocet riadkov vydelime 7mimi, kedze kazda strofa obsahuje 6 informacii + prazdny riadok a dam +1 kvoli poslednej strófe informacii, kde sa nenachadza posledny volny riadok, a teda po vydeleni sa to zaokruhli na cislo o 1 mensie nez potrebujeme*/
        rewind(*f);

        if (*n_was_open == 1) {                         /*Ak n funkcia uz bola zavolana, dealokujeme uz nacitanu pamat*/
            for (int i = 0;
                 i < *total; i++) {                     /*najprv dealokujem "mensie" prvky, teda tie co sa nachadzaju v "poli"*/
                free((*rc)[i]);
                free((*modul)[i]);
                free((*velicina)[i]);
                free((*hodnota)[i]);
        ...
        *rc = (char **) malloc(*total * sizeof(char *));        /*kazdemu polu naalokujem pamat o velkosti total (kolko krat sa dana informacia nachadza vo file a teda budeme potrebovat tolko miesta) vynasobene velkostou characteru*/
        ...
        *datum = (char **) malloc(*total * sizeof(char *));
        for (int i = 0; i < *total; i++) {
            (*rc)[i] = (char *) malloc(12 *sizeof(char));           /*kazdemu prvku v poli naalokujem pamat o velkosti 12*velkost characteru (12 preto, lebo v hocijakom riadku je max 10 znakov + znak konca riadku*/
        ...
        while (n <
            ...
            n++;
            while ((fgetc(*f) != '\n') && (n != *total));  /*posununtie na novy riadok vo file, nefungovalo pri konci suboru a preto druha podmienka*/
        }
        *n_was_open = 1;
    }
    rewind(*f);
    ...
    if (*n_was_open == 0) {
        ...
        for (int n = 0; n < *total; n++) {      /*prechadza vsetky casy z filu*/
            ...
            for (int j = 0; j < pocitadlo; j++) {
                if (strncmp((*cas)[n], polec[j], 4) == 0) {     /*ak sa najde prvok ktory uz je zapisany vo file, pravdivost sa zmeni na 0, tym padom sa nevykona dalsia podmienka nizsie*/
                    pravdivost = 0;
                    break;
                }
            }
            if (pravdivost == 1) {                              /*ak sa nasiel prvok ktory este nie je zapisany v poli polec, tak sa don zapise*/
        ...
        for (int i = 0; i < pocitadlo; i++) {
            int cislo = atoi(polec[i]);                     /* z unikatneho cisla v poli spravim hodinu a minuty a zapisem do pola*/
            poleh[i] = (cislo / 100);
            polem[i] = (cislo % 100);
        }
        for (int counter = 1; counter < 25; counter++) {
            int pravdivost = 0;
            for (int j = 0; j < pocitadlo; j++) {           /*ak sa v poli nachadza hodina rovna counteru (ide od 0hodiny po 24 hodinu)*/
                if (poleh[j] == counter) {
                    printf("%d:", poleh[j]);
                    pravdivost = 1;
                    break;
    ...
    if (*n_was_open == 0) {
        ...
        for (int n = 0; n < *total; n++) {
            if (strncmp(buffer, ((*velicina)[n]), 2) == 0) {
                ...
                if (cislo > maximum) maximum = cislo;               /*zistujem ake najvacsie cislo je vo fily aby som podla toho mohol spravit intervaly po 5tich*/
        ...
        for (int i = 0; i < pocitadlo; i++) {
            int k = 5;
            int pozicia = 0;
            for (int j = 0; j < max; j += 5) {                      /*tu budem zistovat do akeho rangu patri nasa najdena hodnota*/
                float cislo = hodnoty[i];
                if ((j < cislo) && (cislo < k)) {                   /*ak sa hodnota nachadza v danom rangy, tak v phodnot na prislusnom poli sa pripocita +1 na counter, ak sa nenachadza, forcyklus ide dalej a meni sa range aj pozicia*/
                    phodnot[pozicia]++;
                } else {
                    k += 5;
                    pozicia++;
...
void c_funkcia(FILE **f, int *v_was_open) {
    ...
    if (*v_was_open == 0) {
        ...
        while ((znak = (fgetc(*f))) != EOF) {               /*kym nie sme na konci subora*/
            if (pc == 0) ungetc(znak, *f);              /*kedze fgetc nas posunie na zaciatku o 1 znak dalej, tak sa musime vratit naspat ale iba jeden krat, takze premenna pc to zabezpeci*/
            pc++;
            if (counter != 7) {
                switch (counter) {
                    case 1: {
...
void z_funkcia(FILE **f, char ***rc, char ***modul, char ***velicina, char ***hodnota, char ***cas, char ***datum,
    ...
    *z_was_open = 1;
    ...
    if (*n_was_open == 0) printf("Polia nie su vytvorene");
        ...
        scanf("%s", buffer);
        for (int n = 0; n < *total; n++) {
            if (strncmp(buffer, ((*rc)[n]), 10) == 0) {                     /*ak sa najdena hodnota rovna zadanej, tak sa vsetky tieto hodnoty na danej pozicii prepisu na NULL*/
        ...
        int premena = 0;
        for (int n = 0; n < *total; n++) {
            if (strncmp((*rc)[n], "NULL", 4) == 0);         /*idem v range vsetkych nameranych (57), ak sa hodnota na pozicii [i] nerovna NULL, tak sa ponecha na danom mieste, ak sa rovna NULL, tak sa na jej poziciu prepise hodnota z*/
        ...
        *rc = realloc(*rc, (*total - *zcounter) * sizeof(char *));              /* potom reallocujeme na pocet total (57) - counter (pocet, ktory sme vymazali)*/
...
void s_funkcia(FILE **f, int *n_was_open, int *total, char ***modul, char ***velicina, char ***hodnota, char ***datum,
    ...
    if (*n_was_open == 0) {
        ...
        getchar();
        for (int n = 0; n < *total; n++) {
            if ((strncmp(bufferM, (*modul)[n], 3) == 0) && (strncmp(bufferV, (*velicina)[n], 2) == 0))  /*zistim kolko takych mer modulov a velicin sa nachadza v poli co splnaju podmienky*/
                pocitadlo++;
        }
        if(pocitadlo==0) printf("Pre dany vstup neexistuju zaznamy");
            ...
            for (int n = 0; n < *total; n++) {
                if ((strncmp(bufferM, (*modul)[n], 3) == 0) && (strncmp(bufferV, (*velicina)[n], 2) == 0)) {
                    ...
                    for (int j = 0; j < 12; j++) {              /*prvych o znakov to bude brat z datumu, dalsie 4 z casu a tak vznikne string spojenych datumov a casu (nevedno o funkcii strcat)*/
                        if (j < 8) {
...
void o_funkcia(FILE **f, int *n_was_open, int *total, char ***modul, char ***velicina, char ***hodnota, char ***datum,
    ...
    if (*v_was_open == 0) printf("Neotvoreny subor");
        ...
        if (*n_was_open == 0) {                         /*na tuto poslednu cast projektu (vypisovanie priamo zo suboru) nezostal cas, ale z dynamickych poli som stihol*/
            printf("Polia nie su  vytvorene");
        } else {
            scanf("%s %s", bufferM, bufferV);
            getchar();
