...
BeginPackage["GeneratorRandomGraph`"]
Equation::usage = "Equation[difficulty,directed] generates a Random graph problem with solution steps and final result.";
...
(********************************************************************************************************************************)
makeRandomGraph[diff_,directed_]:=Module[{adjacencyMatrix,edges,weights,edgeLabels,mygraph,vertexABC,vertexPairs,selectedEdges,n,edgesNumber},

If[diff===EASY,
n=Input["Enter number of vertexes (8 recommended)"];
edgesNumber=Input["Enter number of edges (12 recommended)"];
];
If[diff===MEDIUM,
n=Input["Enter number of vertexes (10 recommended)"];
edgesNumber=Input["Enter number of edges  (15 recommended)"];
];
If[diff===HARD,
n=Input["Enter number of vertexes (12 recommended)"];
edgesNumber=Input["Enter number of edges (22 recommended)"];
];
adjacencyMatrix=ConstantArray[0,{n,n}];

If[directed===False,
vertexPairs=Flatten[Table[{i,j},{i,1,n},{j,1,n}],1];
selectedEdges=RandomSample[vertexPairs,Min[edgesNumber,Length[vertexPairs]]];
Do[
{i,j}=edge;adjacencyMatrix[[i,j]]=RandomInteger[{1,10}];
...
If[directed===True,
vertexPairs=Flatten[Table[{i,j},{i,1,n},{j,i+1,n}],1];
selectedEdges=RandomSample[vertexPairs,Min[edgesNumber,Length[vertexPairs]]];
Do[
{i,j}=edge;
...
{mygraph,edges,weights,edgeLabels,vertexABC,n}=makeRandomGraph[difficulty,directed];
vertexPositions = GraphEmbedding[mygraph];
