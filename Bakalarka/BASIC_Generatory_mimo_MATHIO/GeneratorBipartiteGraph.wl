...
BeginPackage["GeneratorBipartiteGraph`"]
Equation::usage = "Equation[difficulty,directed] generates a Bipartite graph problem with solution steps and final result.";
...
(********************************************************************************************************************************)
makeBipartiteGraph[diff_,directed_]:=Module[{mygraph,edges,weights,edgeLabels,vertexABC,n1,n2,adjacencyMatrix,edgeList,vertexCoordinates,randomProb,selectedEdges,possibleEdges},

If[diff===EASY,
n=Input["Enter number of vertexes (8 recommended)"];
edgesNumber=Input["Enter number of vertexes (16 recommended)"];
];
If[diff===MEDIUM,
n=Input["Enter number of vertexes (11 recommended)"];
edgesNumber=Input["Enter number of vertexes (20 recommended)"];
];
If[diff===HARD,
n=Input["Enter number of vertexes (14 recommended)"];
edgesNumber=Input["Enter number of vertexes (20 recommended)"];
];
...
possibleEdges=Flatten[Table[{i,k},{i,1,n1},{k,n2,n}],1];
selectedEdges=RandomSample[possibleEdges,Min[edgesNumber,Length[possibleEdges]]];

If[directed===False,
Do[adjacencyMatrix[[edge[[1]],edge[[2]]]]=RandomInteger[{1,10}],{edge,selectedEdges}];

edges=Flatten[Table[If[adjacencyMatrix[[i,j]]>0,{i\[UndirectedEdge]j},Nothing],{i,1,n},{j,i+1,n}]];
weights=Flatten[Table[If[adjacencyMatrix[[i,j]]>0,{adjacencyMatrix[[i,j]]},Nothing],{i,1,n},{j,i+1,n}]];
...
randomProb=0.4;
Do[
If[RandomReal[]<randomProb,
adjacencyMatrix[[i,k]]=RandomInteger[{1,10}];
]
,{i,1,n1},{k,n2,n}
];
Do[
If[RandomReal[]<randomProb,
adjacencyMatrix[[k,i]]=RandomInteger[{1,10}];
]
,{i,1,n1},{k,n2,n}
];
...
vertexCoordinates=Table[If[i<=n1,{-1,-i},{1,-(i-n1)} ],{i,1,n}];
...
{mygraph,edges,weights,edgeLabels,vertexABC,n}=makeBipartiteGraph[difficulty,directed];
vertexPositions = GraphEmbedding[mygraph];
